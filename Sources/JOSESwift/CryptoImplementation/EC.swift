...
extension P256.Signing.PrivateKey {
  func sign(_ signingInput: Data) throws -> Data {
    ...
    do {
      ...
      let varlenR = try Data(ecSignature.read(.integer))
      let varlenS = try Data(ecSignature.skip(.integer).read(.integer))
      let fixlenR = EC.Asn1IntegerConversion.toRaw(varlenR, of: ECCurveType.P256.coordinateOctetLength)
      let fixlenS = EC.Asn1IntegerConversion.toRaw(varlenS, of: ECCurveType.P256.coordinateOctetLength)
...
extension P256.Signing.PublicKey {
  func verify(_ verifyingInput: Data, against signature: Data) throws -> Bool {
    ...
    // pack raw signature as specified for JWS into BER encoded ASN.1 format
    let fixlenR = signature.prefix(curveType.coordinateOctetLength)
    let varlenR = [UInt8](EC.Asn1IntegerConversion.fromRaw(fixlenR))
    let fixlenS = signature.suffix(curveType.coordinateOctetLength)
    let varlenS = [UInt8](EC.Asn1IntegerConversion.fromRaw(fixlenS))
    let asn1Signature = Data((varlenR.encode(as: .integer) + varlenS.encode(as: .integer)).encode(as: .sequence))

    let sig = try P256.Signing.ECDSASignature(derRepresentation: asn1Signature)

    return isValidSignature(sig, for: verifyingInput)
  }
}
...
extension P384.Signing.PrivateKey {
  func sign(_ signingInput: Data) throws -> Data {
    ...
    do {
      ...
      let varlenR = try Data(ecSignature.read(.integer))
      let varlenS = try Data(ecSignature.skip(.integer).read(.integer))
      let fixlenR = EC.Asn1IntegerConversion.toRaw(varlenR, of: ECCurveType.P384.coordinateOctetLength)
      let fixlenS = EC.Asn1IntegerConversion.toRaw(varlenS, of: ECCurveType.P384.coordinateOctetLength)
...
extension P384.Signing.PublicKey {
  func verify(_ verifyingInput: Data, against signature: Data) throws -> Bool {
    let curveType = ECCurveType.P384
    ...
    let sig = try P384.Signing.ECDSASignature(derRepresentation: asn1Signature)
...
extension P521.Signing.PrivateKey {
  func sign(_ signingInput: Data) throws -> Data {
    ...
    do {
      ...
      let varlenR = try Data(ecSignature.read(.integer))
      let varlenS = try Data(ecSignature.skip(.integer).read(.integer))
      let fixlenR = EC.Asn1IntegerConversion.toRaw(varlenR, of: ECCurveType.P521.coordinateOctetLength)
      let fixlenS = EC.Asn1IntegerConversion.toRaw(varlenS, of: ECCurveType.P521.coordinateOctetLength)
...
extension P521.Signing.PublicKey {
  func verify(_ verifyingInput: Data, against signature: Data) throws -> Bool {
    let curveType = ECCurveType.P521
    ...
    let sig = try P521.Signing.ECDSASignature(derRepresentation: asn1Signature)
